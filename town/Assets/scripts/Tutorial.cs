...
public class Tutorial : MonoBehaviour {
    ...
    IEnumerator TutorialSequence()
    {
        ...
        yield return StartCoroutine(Conversation(rachel, "너 왔구나! 진짜 오랜만이야! 나 기억해?\n너 어렸을 때 나랑 같이 여기서 스키타고 놀았잖아."));

        yield return StartCoroutine(Conversation(chris, "그래! 그 때 여기서 살고 있던 남자애..\n그게 나야. 반갑다 정말!"));
        
        yield return StartCoroutine(Conversation(rachel, "계속 여기에 살고 있느냐고? 맞아.\n각 섬마다 관리인이 있는데, 내가 메리스노우의 관리인을 맡았어."));
        ...
        yield return StartCoroutine(Conversation(chris, "아참참. 제가 너무 들떴나봐요. 그럼 저는 제설작업 하던거\n마저 하러 가보겠습니다."));
        
        yield return StartCoroutine(Conversation(rachel, "좀 이따가 봐!"));

        FadeOut(chris);

        yield return StartCoroutine(Conversation(rachel, "먼저, 건물 짓는 방법을 알려드릴게요."));
        yield return StartCoroutine(Conversation(rachel, "상점을 터치하세요."));

        AllFadeOut();
        AllCutOut();

        CutIn(mark1);
        mark1.GetComponent<Animator>().Play("Left Right");
        ...
        CutIn(mark2);
        mark2.GetComponent<Animator>().Play("Up Down");
        ...
        yield return StartCoroutine(Conversation(rachel, "'원숭이 온청장'을 구매하세요."));
        CutOut(rachel);
        CutOut(balloon);
        CutOut(arrow);
        CutOut(mask);
        ...
        CutIn(mark3);
        mark3.GetComponent<Animator>().Play("Up Down");
        ...
        CutIn(arrow);
        CutIn(mask);
        yield return StartCoroutine(Conversation(rachel, "표시된 구역에 건물을 드래그해서 건설을 시작해주세요."));
        CutOut(rachel);
        CutOut(balloon);
        CutOut(arrow);
        CutOut(mask);
        ...
        CutIn(arrow);
        CutIn(mask);
        yield return StartCoroutine(Conversation(rachel, "건물을 구매하면 일정시간의 공사 기간을 거쳐야 건물사용이 가능합니다."));
        ...
        CutIn(arrow);
        CutIn(mask);
        yield return StartCoroutine(Conversation(rachel, "건물 짓기 튜토리얼이 끝났습니다."));
        yield return StartCoroutine(Conversation(rachel, "다음 단계로 넘어가 보실까요?"));
        CutOut(rachel);
        CutOut(balloon);
        CutOut(arrow);
        CutOut(mask);
